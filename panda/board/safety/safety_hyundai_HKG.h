const int HYUNDAI_MAX_STEER = 409;             // like stock  def:384
...
const CanMsg HYUNDAI_TX_MSGS[] = {
 ...
 const CanMsg HYUNDAI_LEGACY_LONG_TX_MSGS[] = {
  {832, 0, 8},  // LKAS11 Bus 0
  {1265, 0, 4}, // CLU11 Bus 0
  {1157, 0, 4}, // LFAHDA_MFC Bus 0
  {593, 2, 8},  // MDPS12, Bus 2      ->0  OP_SCC_live  
...
static int hyundai_rx_hook(CANPacket_t *to_push) {
  ...
  if (valid && (bus == 0U)) {
    ...
    if ((hyundai_longitudinal == 1) && (addr == 1057)) {  // SCC12
...
#ifdef ALLOW_DEBUG
  if( GET_FLAG(param, HYUNDAI_PARAM_LONGITUDINAL) ) hyundai_longitudinal = 1;
#endif
...
static const addr_checks* hyundai_legacy_init(int16_t param) {
  ...
  if( GET_FLAG(param, HYUNDAI_PARAM_LONGITUDINAL) )  hyundai_longitudinal = 2;
